  ...
  try {
    ...
    dashboardData.metrics.forEach(metric => {
      try {
        ...
        const chartId = `${metric.toLowerCase().replace(/\s+/g, '-').replace(/[()]/g, '')}-trend-chart`;
      ...
      Object.keys(dashboardData.structured[initiative]).forEach(subInitiative => {
        Object.keys(dashboardData.structured[initiative][subInitiative]).forEach(metric => {
          try {
            ...
            // Convert metric name to kebab-case, removing parentheses for EMV (USD)
            const metricKebab = metric.toLowerCase().replace(/\s+/g, '-').replace(/[()]/g, '');
            
            const chartId = `initiative-cards-${subInitiativeKebab}-${metricKebab}-chart`;
