// Influencers Dashboard - Chart Configurations and Data Processing
// This file handles data processing and chart creation for the Influencers KPI Dashboard

console.log('Influencers.js loaded successfully');
...
function calculateTotals(data) {
  ...
  // Initialize totals object with all metrics
  const totals = {};
...
function formatNumber(number, metric) {
  ...
  // Special formatting for EMV (USD) - add $ sign
  if (metric === 'EMV (USD)') {
        ...
        label: function(context) {
          ...
          if (context.parsed.y !== null) {
            ...
            if (typeof formatNumber === 'function') {
              label += formatNumber(context.parsed.y, context.dataset.metric);
            } else {
...
function createActualVsForecastChart(canvasId, data, initiative, subInitiative, metric) {
  try {
    ...
    const subInitiativeColors = {
      'Influencers': CHART_COLORS.purple
    };
        ...
        datasets: [
          {
            ...
            tension: 0.3,
            fill: false,
            metric: metric // Add metric for tooltip formatting
          },
          {
            label: 'Forecast',
            data: forecastData,
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ...
          min: 0,
          max: 200, // Increased from 120 to better visualize values that might be higher
          grid: {
